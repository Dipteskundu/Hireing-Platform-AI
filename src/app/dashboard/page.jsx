...
import { User, Briefcase, Shield, Users, AlertCircle, Loader2 } from "lucide-react";
    ...
    useEffect(() => {
        ...
        const fetchAll = async () => {
            ...
            try {
                ...
                if (!profileRes.ok) {
                    throw new Error(`Profile request failed with ${profileRes.status}`);
                }
                const profileData = await profileRes.json();
                const profile = profileData.data || {};
                setUserProfile(profile);
                if (profileData.fallback && profileData.message) {
                    setInfoMessage(profileData.message);
                }
                ...
                if (!dashRes.ok) {
                    throw new Error(`Dashboard request failed with ${dashRes.status}`);
                }
                const dashJson = await dashRes.json();
                setDashboardData(dashJson.data || null);
                if (dashJson.fallback) {
                    setInfoMessage("Showing temporary fallback dashboard data while the database reconnects.");
                }
            } catch (err) {
        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-5 md:pt-5">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <div className="flex items-center gap-2 mb-0.5">
                                    <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full border ${roleConfig.bg} ${roleConfig.color} ${roleConfig.border}`}>
                ...
                {infoMessage && !loading && !error && (
                    <div className="mb-6 flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl text-blue-800 text-sm font-medium">
                        <AlertCircle className="w-5 h-5 shrink-0 mt-0.5 text-blue-500" aria-hidden="true" />
                        {infoMessage}
                    </div>
                )}
